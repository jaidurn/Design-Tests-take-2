...
void World::renderRooms()
{
	for(unsigned int i = 0; i < m_rooms.size(); i++)
	{
		...
		/*Vector2D topLeft{ m_rooms[i]->rect()->getTopLeft() };
		Vector2D topRight{ m_rooms[i]->rect()->getTopRight() };
		Vector2D bottomLeft{ m_rooms[i]->rect()->getBottomLeft() };
		Vector2D bottomRight{ m_rooms[i]->rect()->getBottomRight() };

		Line top{ topLeft, topRight };
		Line bottom{ bottomLeft, bottomRight };
		Line left{ topLeft, bottomLeft };
		Line right{ topRight, bottomRight };

		SDL_Color red{ 255, 0, 0, 255 };

		RenderSystem::instance()->drawLine(top, red);
		RenderSystem::instance()->drawLine(bottom, red);
		RenderSystem::instance()->drawLine(left, red);
		RenderSystem::instance()->drawLine(right, red);

		SDL_Color green{ 0, 255, 0, 255 };

		for (unsigned int j = 0; j < m_hallways.size(); j++)
		{
			Line top = { m_hallways[j]->rect()->getTopLeft(), m_hallways[j]->rect()->getTopRight() };
			Line bottom = { m_hallways[j]->rect()->getBottomLeft(), m_hallways[j]->rect()->getBottomRight() };
			Line left = { m_hallways[j]->rect()->getTopLeft(), m_hallways[j]->rect()->getBottomLeft() };
			Line right = { m_hallways[j]->rect()->getTopRight(), m_hallways[j]->rect()->getBottomRight() };

			RenderSystem::instance()->drawLine(top, green);
			RenderSystem::instance()->drawLine(bottom, green);
			RenderSystem::instance()->drawLine(left, green);
			RenderSystem::instance()->drawLine(right, green);
		}*/
		...
		/*for(int j = 0; j < m_rooms[i]->doorCount(); j++)
		{
			...
			if(m_rooms[i]->getDoor(j)->connected)
			{
				Room *room2 = m_rooms[i]->getDoor(j)->connectedRoom;

				float x1 = m_rooms[i]->rect()->center().getX() - (m_rooms[i]->rect()->width() / 2);
				float y1 = m_rooms[i]->rect()->center().getY() - (m_rooms[i]->rect()->height() / 2);
				float x2 = room2->rect()->center().getX() - (room2->rect()->width() / 2);
				float y2 = room2->rect()->center().getY() - (room2->rect()->height() / 2);

				Vector2D point1(x1, y1);
				Vector2D point2(x2, y2);

				Door *connection = m_rooms[i]->getDoor(j)->connection;

				if (connection->m_position.getX() == 0 && connection->m_position.getY() == 0)
				{
				}
				else
				{
					Vector2D door1 = point1 + m_rooms[i]->getDoor(j)->m_position;
					Vector2D door2 = point2 + m_rooms[i]->getDoor(j)->connection->m_position;

					Line doorway{ door1, door2 };

					RenderSystem::instance()->drawLine(doorway, green);
				}
			}

			//Line hall{ m_rooms[i]->rect().center(), m_rooms[i]->getConnection(j)->rect().center() };
			//Line vertHall{ m_rooms[i]->rect().center(), Vector2D(m_rooms[i]->rect().center().getX(), m_rooms[i]->getConnection(j)->rect().center().getY())  };
			//Line horiHall{ Vector2D(m_rooms[i]->rect().center().getX(), m_rooms[i]->getConnection(j)->rect().center().getY()), m_rooms[i]->getConnection(j)->rect().center() };

			//RenderSystem::instance()->drawLine(hall, green);
			//RenderSystem::instance()->drawLine(vertHall, green);
			//RenderSystem::instance()->drawLine(horiHall, green);
		}*/
